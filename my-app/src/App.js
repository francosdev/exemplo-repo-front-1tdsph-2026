...
function PixelStarField() {
  useEffect(() => {
    ...
    const COLORS = ["#ffffff","#00d4ff","#bb86fc","#ffd700","#4fc3f7","#c0e8ff"];
    ...
    const draw = () => {
      ...
      stars.forEach(s => {
        ...
        ctx.fillStyle = s.col;
        ctx.globalAlpha = alpha * 0.85 + 0.08;
        // pixel square — no smoothing
        ctx.fillRect(Math.floor(s.x), Math.floor(s.y), s.size, s.size);
        // cross sparkle on brighter stars
        if (s.size === 2 && alpha > 0.75) {
          ctx.globalAlpha = (alpha - 0.75) * 2;
          ctx.fillRect(Math.floor(s.x) + 1, Math.floor(s.y) - 1, 1, 1); // N
          ctx.fillRect(Math.floor(s.x) + 1, Math.floor(s.y) + 2, 1, 1); // S
          ctx.fillRect(Math.floor(s.x) - 1, Math.floor(s.y) + 1, 1, 1); // W
          ctx.fillRect(Math.floor(s.x) + 3, Math.floor(s.y) + 1, 1, 1); // E
        }
      });
      ctx.globalAlpha = 1;
...
═══════════════════════════════════════════════════════════ */
const MOODS = [
  { emoji: "😄", label: "BLESSED",  value: 5, color: "#39ff14" },
  { emoji: "🙂", label: "GOOD",     value: 4, color: "#00d4ff" },
  { emoji: "😐", label: "NEUTRAL",  value: 3, color: "#ffd700" },
  { emoji: "😔", label: "CURSED",   value: 2, color: "#f97316" },
  { emoji: "😢", label: "DOOMED",   value: 1, color: "#ff5555" },
];

const GUILDS = [
  { id: "leitura",    label: "LORE ORDER",   icon: "📚", color: "#bb86fc" },
  { id: "estudos",    label: "MAGE ACADEMY", icon: "🎓", color: "#00d4ff" },
  { id: "saude",      label: "DRUID HALL",   icon: "🌿", color: "#39ff14" },
  { id: "financeiro", label: "MERCHANT CLAN",icon: "💰", color: "#ffd700" },
  { id: "pessoal",    label: "HERO CIRCLE",  icon: "⭐", color: "#ff79c6" },
  { id: "trabalho",   label: "STRATEGISTS",  icon: "🛡️", color: "#4fc3f7" },
];

const QUEST_TYPES = [
  { value: "lendaria",  label: "LEGENDARY", color: "#ffd700", xp: 200, icon: "👑" },
  { value: "epica",     label: "EPIC",      color: "#bb86fc", xp: 100, icon: "💜" },
  { value: "normal",    label: "NORMAL",    color: "#00d4ff", xp:  50, icon: "🔵" },
  { value: "secundaria",label: "SIDE",      color: "#39ff14", xp:  25, icon: "🌿" },
];
...
const getClass  = g  => { if(!g.length)return{title:"ARCANE ARCHER",icon:"🏹"}; const c={}; g.forEach(x=>c[x.category]=(c[x.category]||0)+1); const t=Object.entries(c).sort((a,b)=>b[1]-a[1])[0][0]; return ({leitura:{title:"LORE KEEPER",icon:"📚"},estudos:{title:"ARCH MAGE",icon:"🧙‍♀️"},saude:{title:"DRUID",icon:"🌿"},financeiro:{title:"MERCHANT",icon:"💰"},pessoal:{title:"HERO",icon:"⭐"},trabalho:{title:"PALADIN",icon:"🛡️"}})[t]||{title:"ADVENTURER",icon:"🏹"}; };
const getTitle  = l  => l>=20?"ANCIENT ONE":l>=15?"CHAMPION":l>=10?"GUARDIAN":l>=7?"WARRIOR":l>=4?"SQUIRE":"RECRUIT";
  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"10px", marginBottom:"16px" }}>
        {[
          { icon:"📖", label:"CHRONICLE",  val:Object.keys(diary).length, sub:`${weekEntries} this week`, color:"#bb86fc" },
          { icon:"⚔️", label:"QUESTS DONE",val:done,   sub:`${pending} open`,    color:"#39ff14" },
          { icon:"☠️", label:"OVERDUE",    val:overdue, sub:overdue>0?"DANGER!":"ALL CLEAR", color:overdue>0?"#ff5555":"#39ff14" },
          { icon:"🏆", label:"FEATS AVG",  val:`${avgG}%`, sub:`${goals.length} active`,   color:"#ffd700" },
        ].map(s => (
      ...
      {goals.length>0 && (
        <div className="px-panel">
          <div style={{ position:"absolute", top:-6, right:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
          <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
          <PxTitle icon="🏆" color="#ffd700">FEATS IN PROGRESS</PxTitle>
          <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
            {goals.map(g=>{
      ...
      {tasks.filter(t=>!t.completed).length>0 && (
        <div className="px-panel">
          <div style={{ position:"absolute", top:-6, right:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
          <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
          <PxTitle icon="📜" color="#00d4ff">ACTIVE QUESTS</PxTitle>
          <div style={{ display:"flex", flexDirection:"column", gap:"6px" }}>
            ...
            {tasks.filter(t=>!t.completed).length>5 && (
              <div className="px-body" style={{ fontSize:"16px", color:"#1a4070", textAlign:"center", padding:"4px" }}>
                +{tasks.filter(t=>!t.completed).length-5} more quests...
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div>
      <div className="px-panel-hero">
        <div style={{ position:"absolute", top:-6, right:-6, width:8, height:8, background:"#020a1e", border:"2px solid #ffd700", zIndex:2 }} />
        <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #ffd700", zIndex:2 }} />
        <PxTitle icon="📖" color="#ffd700">CHRONICLE — {fmtDate(new Date())}</PxTitle>

        <PxLabel color="#bb86fc">SELECT YOUR AURA STATUS</PxLabel>
        ...
        <PxLabel color="#00d4ff">TODAY'S DEEDS</PxLabel>
        <input className="px-input" style={{ marginBottom:"14px" }}
          placeholder="> Completed 2 quests, studied magic, trained..."
          value={entry.activities||""}
          onChange={e=>upd("activities",e.target.value)}
        />

        <PxLabel color="#39ff14">ADVENTURER'S REFLECTIONS</PxLabel>
        <textarea className="px-textarea"
          placeholder="> How was your journey today? What did your heart feel?"
          value={entry.text||""}
          onChange={e=>upd("text",e.target.value)}
        />
        <div style={{ marginTop:"10px", display:"flex", alignItems:"center", gap:"8px" }}>
          <span className="px-blink" style={{ color:"#39ff14", fontSize:"12px" }}>■</span>
          <span className="px-font" style={{ fontSize:"7px", color:"#39ff14" }}>AUTO-SAVED · +20 XP PER ENTRY</span>
        </div>
      </div>
      ...
      {list.length>1 && (
        <div className="px-panel">
          <div style={{ position:"absolute", top:-6, right:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
          <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
          <PxTitle icon="🗓️" color="#4fc3f7">PAST RECORDS</PxTitle>
  ...
  return (
    <div>
      <div className="px-panel">
        <div style={{ position:"absolute", top:-6, right:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
        <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
        <PxTitle icon="📜" color="#00d4ff">ACCEPT NEW QUEST</PxTitle>
        <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
          <input className="px-input" placeholder="> Quest name..."
            value={form.title}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"10px" }}>
            {[
              { label:"DIFFICULTY", child:<select className="px-input" value={form.priority} onChange={e=>setForm(p=>({...p,priority:e.target.value}))}>{QUEST_TYPES.map(q=><option key={q.value} value={q.value}>{q.icon} {q.label}</option>)}</select> },
              { label:"DEADLINE",   child:<input type="date" className="px-input" value={form.dueDate} onChange={e=>setForm(p=>({...p,dueDate:e.target.value}))} /> },
              { label:"GUILD",      child:<select className="px-input" value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value}))}>
          ...
          <button className="px-btn px-btn-cyan" onClick={add}>⚔ ACCEPT QUEST</button>
        </div>
      </div>
      ...
      {tasks.length>0 && (
        <div className="px-panel">
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"12px", flexWrap:"wrap", gap:"8px" }}>
            <span className="px-font" style={{ fontSize:"7px", color:"#1a4070" }}>{doneCt}/{tasks.length} COMPLETE</span>
            <div style={{ display:"flex", gap:"6px" }}>
              {[["all","ALL"],["pending","ACTIVE"],["done","DONE"]].map(([f,l])=>(
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
            {list.length===0 && <div className="px-body" style={{ textAlign:"center", color:"#1a4070", padding:"20px", fontSize:"18px" }}>NO QUESTS IN THIS FILTER...</div>}
            {list.map(q=>{
              const qt  = QUEST_TYPES.find(t=>t.value===q.priority)||QUEST_TYPES[2];
      ...
      {tasks.length===0 && (
        <div className="px-panel" style={{ textAlign:"center", padding:"40px" }}>
          ...
          <div className="px-font" style={{ fontSize:"9px", color:"#1a4070", marginBottom:"8px" }}>QUEST BOARD EMPTY</div>
          <div className="px-body" style={{ fontSize:"18px", color:"#0d2860" }}>Accept your first quest, adventurer!</div>
        </div>
      )}
    </div>
  );
...
function GrimorioModule({ notes, setNotes }) {
  ...
  const create = () => { const n={id:uid(),title:"NEW SPELL",content:"",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}; setNotes(p=>[n,...p]); setSel(n.id); };
    ...
    <div style={{ display:"grid", gridTemplateColumns:"260px 1fr", gap:"16px", minHeight:"500px" }}>
      <div>
        <div style={{ display:"flex", gap:"8px", marginBottom:"10px" }}>
          <input className="px-input" style={{ flex:1, fontSize:"15px" }} placeholder="> search..." value={search} onChange={e=>setSearch(e.target.value)} />
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"6px", maxHeight:"580px", overflowY:"auto" }}>
          {list.length===0 && <div className="px-body" style={{ color:"#0d2860", textAlign:"center", padding:"20px", fontSize:"18px" }}>GRIMOIRE EMPTY...</div>}
          {list.map(n=>(
            <div key={n.id} onClick={()=>setSel(n.id)} style={{
              ...
              <div className="px-font" style={{ fontSize:"7px", color:sel===n.id?"#bb86fc":"#1a4070", marginBottom:"4px" }}>📜 {n.title||"UNTITLED"}</div>
              <div className="px-body" style={{ fontSize:"14px", color:"#0d2860" }}>{n.content?n.content.slice(0,55)+"...":"[empty]"}</div>
      ...
      <div className="px-panel-arcane" style={{ minHeight:"500px" }}>
        ...
        {cur
          ? <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"14px", gap:"10px" }}>
                <input style={{ background:"transparent", border:"none", borderBottom:"3px solid #bb86fc44", color:"#bb86fc", fontFamily:"'Press Start 2P',monospace", fontSize:"10px", flex:1, padding:"0 0 8px 0", outline:"none" }}
                  value={cur.title} onChange={e=>upd(cur.id,"title",e.target.value)} placeholder="SPELL NAME..." />
              ...
              <textarea style={{ background:"transparent", border:"none", color:"#c0e8ff", fontFamily:"'VT323',monospace", fontSize:"18px", width:"100%", minHeight:"400px", resize:"none", outline:"none", lineHeight:1.7 }}
                ...
                placeholder="> Write your arcane spells, secrets and ancient wisdom here..." />
            </>
          : <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"400px", gap:"12px" }}>
              ...
              <span className="px-font" style={{ fontSize:"8px", color:"#1a4070", textAlign:"center", lineHeight:2 }}>SELECT A SPELL<br/>OR CREATE NEW</span>
              <button className="px-btn px-btn-purple" style={{ padding:"12px 20px" }} onClick={create}>✦ NEW SPELL</button>
            </div>
        }
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:"8px", marginBottom:"16px", flexWrap:"wrap", alignItems:"center" }}>
        <button className={`px-btn ${activeG==="all"?"px-btn-gold":"px-btn-ghost"}`} style={{ padding:"8px 12px" }} onClick={()=>setActiveG("all")}>🏆 ALL</button>
        {GUILDS.map(g=>(
        ...
        <button className="px-btn px-btn-gold" style={{ padding:"8px 14px", marginLeft:"auto" }} onClick={()=>setShowForm(!showForm)}>+ NEW FEAT</button>
      </div>
      ...
      {showForm && (
        <div className="px-panel-hero">
          <div style={{ position:"absolute", top:-6, right:-6, width:8, height:8, background:"#020a1e", border:"2px solid #ffd700", zIndex:2 }} />
          <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #ffd700", zIndex:2 }} />
          <PxTitle icon="🏆" color="#ffd700">NEW EPIC FEAT</PxTitle>
          <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
            <input className="px-input" placeholder="> Feat name..." value={form.title} onChange={e=>setForm(p=>({...p,title:e.target.value}))} />
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px" }}>
              {[
                { label:"GUILD", child:<select className="px-input" value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value}))}>{GUILDS.map(g=><option key={g.id} value={g.id}>{g.icon} {g.label}</option>)}</select> },
                { label:"DEADLINE", child:<input type="date" className="px-input" value={form.deadline} onChange={e=>setForm(p=>({...p,deadline:e.target.value}))} /> },
                { label:"TARGET VALUE", child:<input type="number" className="px-input" value={form.targetValue} onChange={e=>setForm(p=>({...p,targetValue:Number(e.target.value)}))} /> },
                { label:"UNIT (%, PAGES, KM...)", child:<input className="px-input" value={form.unit} onChange={e=>setForm(p=>({...p,unit:e.target.value}))} placeholder="%" /> },
              ].map(({label,child})=>(
                <div key={label}><PxLabel>{label}</PxLabel>{child}</div>
              ))}
            </div>
            <textarea className="px-textarea" placeholder="> Description (optional)..." value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} />
            <div style={{ display:"flex", gap:"8px" }}>
              <button className="px-btn px-btn-gold" style={{ padding:"12px 20px" }} onClick={add}>⚔ START FEAT</button>
              <button className="px-btn px-btn-ghost" style={{ padding:"12px 16px" }} onClick={()=>setShowForm(false)}>CANCEL</button>
            </div>
          </div>
        </div>
      )}
      ...
      {list.length===0&&!showForm && (
        <div className="px-panel" style={{ textAlign:"center", padding:"40px" }}>
          ...
          <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #00d4ff", zIndex:2 }} />
          <div className="px-float" style={{ fontSize:"48px", marginBottom:"12px", display:"block" }}>🏆</div>
          <div className="px-font" style={{ fontSize:"8px", color:"#1a4070", marginBottom:"12px" }}>NO FEATS FOUND</div>
          <button className="px-btn px-btn-gold" style={{ padding:"12px 20px" }} onClick={()=>setShowForm(true)}>✦ CREATE FIRST FEAT</button>
        </div>
      )}
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"12px" }}>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"8px", flexWrap:"wrap" }}>
                    ...
                    {done && <PxBadge color="#39ff14">✓ COMPLETE!</PxBadge>}
                  </div>
                  {goal.description && <p className="px-body" style={{ fontSize:"16px", color:"#1a4070", margin:"0 0 10px 0" }}>{goal.description}</p>}
                  <div style={{ marginBottom:"8px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"4px" }}>
                      <span className="px-font" style={{ fontSize:"7px", color:guild?.color||"#00d4ff" }}>PROGRESS</span>
                  ...
                  <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
                    ...
                    {dl!==null && <PxBadge color={dl<7?"#ff5555":"#1a4070"}>{dl>0?`${dl}D LEFT`:dl===0?"TODAY!":"EXPIRED"}</PxBadge>}
...
function OracleModule({ diary, tasks, goals, notes, apiKey, setApiKey }) {
  ...
  const consult = async ()=>{
    if(!apiKey.trim()){setError("INSERT ARCANE KEY (API KEY) TO AWAKEN THE ORACLE.");return;}
...
5. 🌟 **Façanha em Destaque** — Qual merece mais esforço agora`,
    ...
    }catch(e){setError(`ORACLE UNREACHABLE: ${e.message}`);}
  ...
  return (
    <div>
      <div className="px-panel-arcane">
        <div style={{ position:"absolute", top:-6, right:-6, width:8, height:8, background:"#020a1e", border:"2px solid #bb86fc", zIndex:2 }} />
        <div style={{ position:"absolute", bottom:-6, left:-6, width:8, height:8, background:"#020a1e", border:"2px solid #bb86fc", zIndex:2 }} />
        <PxTitle icon="🔮" color="#bb86fc">ORACLE'S CHAMBER</PxTitle>

        <PxLabel color="#bb86fc">ARCANE KEY · ANTHROPIC API KEY</PxLabel>
        ...
        <p className="px-body" style={{ fontSize:"15px", color:"#1a4070", marginBottom:"16px", fontStyle:"italic" }}>
          [KEY STORED LOCALLY · GET YOURS AT{" "}
          <a href="https://console.anthropic.com" target="_blank" rel="noopener noreferrer" style={{ color:"#bb86fc" }}>console.anthropic.com</a>]
        </p>

        <PxLabel color="#bb86fc">SELECT REVELATION TYPE</PxLabel>
        <div style={{ display:"flex", gap:"8px", flexWrap:"wrap", marginBottom:"18px" }}>
          {[
            {id:"semanal",icon:"📊",label:"WEEKLY VISION",  desc:"Full week journey"},
            {id:"humor",  icon:"🌑",label:"AURA READING",   desc:"Emotional & spiritual"},
            {id:"metas",  icon:"⚔️",label:"FEATS MAP",      desc:"Quests & objectives"},
          ].map(t=>(
        ...
        <button className="px-btn" onClick={consult} disabled={loading} style={{
          ...
          {loading?"⌛ ORACLE READS THE STARS...":"✦ INVOKE ORACLE VISION ✦"}
        </button>

        {error && (
          <div style={{ marginTop:"14px", padding:"12px", background:"#200808", border:"3px solid #ff555566" }}>
            <span className="px-font" style={{ fontSize:"7px", color:"#ff5555" }}>☠ ERROR: </span>
      ...
      {analysis && (
        <div className="px-panel-arcane">
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"18px" }}>
            ...
            <div>
              <div className="px-font" style={{ fontSize:"9px", color:"#bb86fc", textShadow:"0 0 10px rgba(187,134,252,.7)" }}>ORACLE REVELATION</div>
...
const SKILL_TREE = [
  ...
  },
  {
    ...
    unlocks: null,
    quests: [
      { id: "ai1", name: "Fundamentos de Inteligência Artificial" },
      { id: "ai2", name: "Machine Learning Básico" },
      { id: "ai3", name: "Processamento de Linguagem Natural (NLP)" },
      { id: "ai4", name: "APIs de IA (OpenAI, Claude)" },
      { id: "ai5", name: "Construção de Chatbot" },
      { id: "ai6", name: "Treinamento e Fine-tuning" },
      { id: "ai7", name: "Ética em IA" },
    ],
  },
  {
    id: "javascript",
...
/* ─── Grupos visuais da árvore (cada grupo = linha na tela) ─── */
const SKILL_ROWS = [
  { left: "python",     right: "chatbot"  }, // 🐍 → 🤖
  { left: "javascript", right: "frontend" }, // ⚡ → 🌐
  { left: "java",       right: null       }, // ☕
  { left: "database",   right: null       }, // 🗄️
  ...
  return (
    <div style={{ maxWidth:"900px", margin:"0 auto" }}>
      ...
      <div className="px-panel" style={{ marginBottom:"24px" }}>
        ...
        <div className="px-font" style={{ fontSize:"8px", color:"#00d4ff", marginBottom:"20px" }}>▶ MAPA DE HABILIDADES</div>
          ...
          {SKILL_ROWS.map((row, i) => {
            ...
            return (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:"0" }}>
                ...
                {row.right && (
                  <>
                    <Arrow color={leftSkill?.color || "#00d4ff"} />
